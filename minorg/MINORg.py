...
class PathHandler:
    def __init__(self, tmp = True, keep_tmp = False, keep_on_crash = False, directory = None):
...
class MINORg (PathHandler):
    def __init__(self, verbose = False, directory = None, config = None, prefix = None,
                 tmp = False, keep_tmp = False, keep_on_crash = False, thread = None):
        self.verbose = False
        super().__init__(directory = directory, tmp = tmp, keep_tmp = keep_tmp, keep_on_crash = keep_on_crash)
        ...
        self.attr_mod = self.params.attr_mod.default
        self.genetic_code = self.params.genetic_code.default
    ...
    def get_ref_seqid(self, seqid, attr = "gene"):
    ...
    def parse_reference(self, reference = None, reference_set = None):
    ...
    def add_reference(self, alias, fasta, annotation, genetic_code = 1, attr_mod = {},
        ...
        '''
        alias: name of fasta-annotation combo
        fasta: path to fasta file
        annotation: path to GFF3 file
        genetic_code (default=1): NCBI genetic code
        attr_mod (default={}): mapping for non-standard attribute field names in GFF3 file
        replace (default=False): if True, existing reference associated with alias will be replaced
        '''
    ...
    def extend_reference(self):
        ...
        extend_reference(self.ext_gene, self.ext_cds, ext_fasta, ext_gff, mafft = self.mafft,
    ...
    def _get_reference_seq(self, ids, *features, adj_dir=False, by_gene=False, mktmp=None, ref=None,
        ...
        return output
    def get_reference_seq(self, *features, adj_dir=False, by_gene=False, mktmp=None, ref=None,
                          seqid_template="Reference|$source|$gene|$isoform|$feature|$n",
                          translate=False, isoform_lvl=None, fout=None,
                          # apply_template_to_dict=False,
                          **fouts):
        '''
        Identical to self._get_reference_seq except ids is autofilled with self.genes
        '''
        return self._get_reference_seq(self.genes, *features, adj_dir = adj_dir,
                                       by_gene = by_gene, mktmp = mktmp, ref = ref,
    ...
    def seq(self, minlen = None, minid = None, mincdslen = None, quiet = True,
            check_recip = None, relax_recip = None, check_id_before_merge = None):
        '''
        Uses self.homologue to generate self.target file based on self's attributes
        '''
        if not self.genes:
    ...
    def adjust_feature_range(self, seq, gene, ref_alias, feature, domain = False, **kwargs):
    ...
    def align_reference(self, fout):
        ...
        for seqid_cds in seqids_cds:
            ...
            adj_ranges = self.adjust_feature_range(seq_gene, gene, source, "CDS",
    ...
    def check_valid_status(self, check_name, type, descr):
        if type == "hit":
    ...
    def minimumset(self, sets = None, manual = False, fasta = None, fout_fasta = None, fout_map = None,
        ...
        self.check_valid_status("GC", "seq", "%GC")
        self.check_valid_status("background", "seq", "off-target")
        self.check_valid_status("feature", "hit", "within-feature")
        ...
        while len(grna_sets) < sets:
            ...
            seq_set = get_minimum_set(grna_hits, set_num = len(grna_sets) + 1, manual_check = manual,
                                      targets = targets)
    ...
    #     self.attr_mod = self.params.attr_mod.default
    #     self.sep = args.sep
    #     self.genetic_code = args.genetic_code
    #     ## homologue params
    #     self.pssmids = [] ## if multiple pssmids are provided, overlapping domains will be combined and output will not distinguish between one pssmid or another
    #     self._domain_name = None ## human-readable name for self.pssmids
    #     self.gff_domain = None
    #     ## previously self.query_map storing [(<alias>, <path>)]. Now: {<alias>: <path>}
    #     ## self.query_map is now @property
    #     self.query = {k: v for k, v in args.query_map}
    #     self.genes = [] ## [<geneA>, <geneB>]
    #     self.query_reference = False ## if True, reference genes will also be queried for gRNA
    #     self.ref_gene = None ## fasta file
    #     self.ref_cds = None ## fasta file
    #     self.minlen = self.params.minlen.default
    #     self.minid = self.params.minid.default
    #     self.mincdslen = self.params.mincdslen.default
    #     self._check_recip = self.params.check_recip.default
    #     self.relax_recip = self.params.relax_recip.default
    #     self.check_id_before_merge = self.params.check_id_premerge.default
    #     self.merge_within = self.params.merge_within.default
    #     ## generate gRNA params
    #     self.target = None ## output by self.seq()
    #     self._length = self.params.length.default
    #     self._pam = PAM(pam = self.params.pam.default, gRNA_length = self.params.length.default)
    #     ## filter params
    #     self.grna_hits = None ## output by self.generate_grna()
    #     self.screen_reference = True
    #     self._offtarget = None ## user-provided function that takes in pam, QueryResult obj, and HSP object and returns whether the off-target hit is acceptable. If not set, defaults to using self.ot_mismatch and self.ot_gap (combined with OR) to set lower limits for acceptable off-target
