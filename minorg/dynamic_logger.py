...
class CustomFormatter(logging.Formatter):
    ...
    def __init__(self, fmt = None, fmt_msg = None, datefmt = None, style = '%'):
        slogger.debug("CustomFormatter super().__init__")
        super().__init__(fmt = fmt, datefmt = datefmt, style = style)
        self._fmt_msg = (lambda x:x) if fmt_msg is None else fmt_msg
    
    ## it appears that this function is not called at all??
    def format(self, record):
        record.msg = self._fmt_msg(record.msg)
...
class PublicLogger(logging.Logger, MultiHandlerBase):
    ...
    def __init__(self, name, unique = True, default_log_level = logging.INFO,
        ...
        self._format_msg = format_msg ## outputs str, takes a tuple/list of args
    ...
    def _log(self, lvl, msg, args, **kwargs):
        super()._log(lvl, self._format_msg(msg), args, **kwargs)
...
class MultiLogger(MultiChild):
    ...
    def __init__(self, logger_class = PublicLogger,
                 make_logger = lambda name, **kwargs: PublicLogger(name, **kwargs),
...
class GroupLogger(MultiLogger):
    ...
    def __init__(self, **kwargs):
        ...
        self._format = MultiChild(child_adj = "formatter", child_obj = "Formatter",
                                  child_class = CustomFormatter,
                                  make_child = (lambda name, format, *args, **kwargs: \
                                                CustomFormatter(format, *args, **kwargs)))
    ...
    def _add_handler(self, name, handler, format = "%(asctime)s %(message)s",
                     level = None, replace = False, quiet = False, **kwargs):
    ...
    def add_format(self, name, format, replace = False, quiet = False, format_msg = None):
...
class PublicAwareLogger(PublicLogger):
    ...
    def __init__(self, name, parent, crosstalk = True, **kwargs):
        self._parent = parent
...
class DynamicFileLoggerBase(GroupLogger):
    ...
    def __init__(self, filename, level = lvl, default_log_level = logging.INFO,
        ...
        GroupLogger.__init__(self, logger_class = PublicAwareLogger,
                             make_logger = lambda name, **kwargs: PublicAwareLogger(name, self, **kwargs),
...
class DynamicMultiFileLogger(GroupLogger):
    ...
    def __init__(self, crosstalk = True):
        ...
        GroupLogger.__init__(self,
                             ...
                             make_logger = (lambda name, **kwargs:
                                            PublicAwareLogger(name, self, crosstalk = crosstalk, **kwargs)))
