...
def valid_aliases(aliases, lookup, raise_error = True, message = None, param = None,
    ...
    Arguments:
        ...
        none_value: optional, special alias value to indicate NO alias(es) are to be looked up.
            (I.e. valid_aliases('none', {'genomeA': '/path/to/FASTA', 'genomeB': '/path/to/FASTA'}, none_value = 'none') will not flag the alias ('none') as invalid despite it not being in the lookup dictionary.)
        all_value: optional, special alias value to indicate ALL alias(es).
            (I.e. valid_aliases('all', {'genomeA': '/path/to/FASTA', 'genomeB': '/path/to/FASTA'}, all_value = 'all') will not flag the alias ('all') as invalid despite it not being in the lookup dictionary.)
...
class MINORgCLI (MINORg):
    ...
    def args(self):
        ...
        :setter: Stores a copy of args at self._raw_args and sets self._args to be acted upon by MINORgCLI parsing functions
        :type: Namespace
        """
        return self._args
    @args.setter
    ...
    def _parse_file_set(self, val: str, lookup: dict, description: str,
        ...
        """
        Accepts an alias/path that points to a tab-separated lookup file (self.reference_set, self.cluster_set, or self.genome_set) and parses that lookup file into a dictionary of {<alias>: <value>}.
    ...
    def references_callback(self, value: bool):
        """
        Callback for printing reference genome lookup mapping to terminal. Immediately after, resolve temporary directory using :meth:`MINORgCLI.cleanup` and exit programme using ``typer.Exit()``.
    ...
    def make_genomes_callback(self, mode: str = "independent") -> Callable[[bool],None]:
        ...
        def genomes_callback(value: bool):
            """
            Callback for printing query genome lookup mapping to terminal. Immediately after, resolve temporary directory using :meth:`MINORgCLI.cleanup` and exit programme using ``typer.Exit()``.
    ...
    def clusters_callback(self, value: bool):
        """
        Callback for printing cluster-members lookup mapping to terminal. Immediately after, resolve temporary directory using :meth:`MINORgCLI.cleanup` and exit programme using ``typer.Exit()``.
    ...
    def members_callback(self, value: str):
        """
        Callback for printing gene members of a cluster to terminal. Immediately after, resolve temporary directory using :meth:`MINORgCLI.cleanup` and exit programme using ``typer.Exit()``.
    ...
    def check_filter_args(self, standalone = True):
        """
        Check arguments for subcommand 'filter' and parse & reformat args for parent :class:`minorg.MINORg.MINORg`.
    ...
    def set_genes(self, gene_set_prefix):
        ...
        As :class:`~minorg.MINORg.MINORg` only processes genes in :attr:`~minorg.MINORg.genes`, this method sets :attr:`~minorg.MINORg.genes` as the genes mapped to by ``gene_set_prefix`` so that they can be acted on.
