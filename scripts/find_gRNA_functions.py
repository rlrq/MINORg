...
def find_cluster_gRNA(target_fname, pam = "GG", gRNA_len = 20): #Let NGG be GG, or NG be G
    """
...
# def write_all_gRNA(fout, target_fname, pam, gRNA_len):
#     print("Identifying possible gRNA sequences")
#     ## all_gRNA is a dictionary of format {<gRNA seq>: set(id of targeted targets)}
#     all_gRNA = find_cluster_gRNA(target_fname, pam = pam, gRNA_len = gRNA_len)    
#     ## write gRNA dictionary to fasta file
#     def write_gRNA_fasta(fout, gRNA_hits, override_seqid = False):
#         output = {}
#         for i, gRNA_seq in enumerate(gRNA_hits.gRNAseqs().values()):
#             if override_seqid or not gRNA_seq.id():
#                 gRNA_seq.set_id(f"gRNA_{i+1}")
#             output[gRNA_seq.id()] = gRNA_seq.seq()
#         # dict_to_fasta(output, fout)
#         all_gRNA.write_fasta(fout, write_all = True)
#         return
#     write_gRNA_fasta(fout, all_gRNA)
#     return all_gRNA
...
# ## write gRNA + their targets to file (gRNA_hits was previous gRNA_dict)
# def write_gRNA_targets(fout, gRNA_hits, fin, write_checks = False,
#                        checks = ["background", "GC", "CDS"], sets = []):
#     if not write_checks:
#         checks = []
#     if not sets: ## if not sets, then let all gRNA seqs belong to set 1
#         sets = [set(itertools.chain(map(str, fasta_to_dict(fin).values())))]
#     mapping_header = '\t'.join(["gRNA id", "gRNA sequence", "target id", "target sense", "gRNA strand", "range (0-index, end exclusive)", "group"] + checks)
#     mapping_dat = []
#     gRNA_seqs_inv = {str(v): k for k, v in fasta_to_dict(fin).items()}
#     for set_num, set_seqs in enumerate(sets):
#         mapping_dat.extend(['\n'.join(sorted(['\t'.join(map(str,
#                                                             [gRNA_seqs_inv[str(gRNA_seq)],
#                                                              str(gRNA_seq), hit.target_id(),
#                                                              hit.parent_sense(mode = "str"), hit.strand(),
#                                                              "[{}, {})".format(*hit.range()), set_num + 1] +
#                                                             [hit.check(check_name, mode = "str")
#                                                              for check_name in checks]))
#                                               for hit in gRNA_hits.get_hits(str(gRNA_seq))]))
#                             for gRNA_seq in set_seqs])
#     mapping_dat = '\n'.join([x for x in mapping_dat if x])
#     mapping_f = open(fout, "w+")
#     mapping_f.write('\n'.join((mapping_header, mapping_dat))+'\n')
#     mapping_f.close()
#     return
...
def filter_background(seqs_fname, target_fname, background_fname, gRNA_hits, screen_reference = False,
    ...
    ## check for presence of PAM downstream of excluded gRNA; if no PAM, remove from excl_seqid
...
## filter within CDS
def filter_in_cds(gRNA_hits, alignment_fname, cds_fasta, complete_fasta, relax = False,
    ...
    cds_ranges = get_CDS_ranges(alignment_fname, cds_titles, complete_titles, relax = relax)
...
def get_CDS_ranges(alignment_fname, cds_titles = [], complete_titles = [], relax = False):
    ...
    for i in range(len(cds_titles)):
        ...
        for i in range(len(cds_seq)):
            if (not in_cds) and (((not relax) and cds_seq[i] != '-') or
                                 (relax and cds_seq[i] == complete_seq[i] != '-')):
                in_cds = True
                range_start = i
            elif in_cds and (((not relax) and cds_seq[i] == '-') or
                             (relax and cds_seq[i] != complete_seq[i])):
                in_cds = False
                cds_ranges[cds_title].append((range_start, i))
            elif in_cds and i == len(cds_seq) -1: #  and \
                 # ((not relax) or cds_seq[i] == complete_seq[i] != '-'):
...
## LAR algorithm
def set_cover_LAR(gRNA_coverage, target_ids, id_key = lambda x: x, tie_breaker = lambda x: x.items()[0]):
