...
def execute_grna(directory, prefix, target, args, config):
    all_gRNA_fname = config.mkfname(directory, f"{prefix}_gRNA_all.fasta")
    all_gRNA = find_cluster_gRNA(target, pam = args.pam, gRNA_len = args.length)
...
def find_cluster_gRNA(target_fname, pam = "NGG", gRNA_len = 20): #Let NGG be GG, or NG be G
    """
